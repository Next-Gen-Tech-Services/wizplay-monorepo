...
export default class UserController {
  ...
  public async updateUserStatusInternal(req: Request, res: Response) {
    try {
      // Validate service secret for internal calls
      const { userId } = req.params;
      const { status } = req.body;
      ...
      console.error("UserController.updateUserStatusInternal error:", err);
      return res.status(STATUS_CODE.INTERNAL_SERVER ?? 500).json({
        success: false,
        message: "Failed to update user status",
        data: null,
