...
import { MessageBrokerType, MessageHandler, PublishType, TOPIC_TYPE } from "./kafka-client.types";
...
export class KafkaClient implements MessageBrokerType {
  ...
  async createTopics(topics: string[]) {
    ...
    if (topicsToCreate.length > 0) {
      ...
      this.logger.info(`Created Topics: ${topicsToCreate.map(t => t.topic).join(", ")}`);
    }
  ...
  async publish(data: PublishType): Promise<boolean> {
    ...
    this.logger.info(`Event Published: ${data.event} | ${data.topic} | ${JSON.stringify(result)}`);
  ...
  async subscribeMultiple(messageHandler: MessageHandler, topics: string[]): Promise<void> {
    const consumer = await this.connectConsumer();
